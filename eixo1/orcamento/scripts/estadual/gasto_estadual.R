#===============================================================================
# SCRIPT: EIXO 1 - ORÇAMENTO (ESTADUAL INTEGRADO)
# 
# INPUTS:
# - data/raw/estadual/msc_orcamentaria_estados_2019_2025_final.parquet (Fonte: SICONFI)
# - data/raw/estadual/RCL_*.csv (Arquivos SICONFI Estaduais)
# 
# OUTPUTS:
# - data/processed/estadual/estadual_final.csv
# - outputs/estadual/grafico_evolucao_nominal.png
# - outputs/estadual/grafico_evolucao_real.png
# - outputs/estadual/grafico_composicao_percentual.png
# - outputs/estadual/painel_esforco_fiscal.png
#===============================================================================
...
path_raw_estadual       <- here("data", "raw", "estadual")
path_processed_estadual <- here("data", "processed", "estadual")
path_outputs_estadual   <- here("outputs", "estadual")

# Cria as pastas caso não existam
dir.create(path_processed_estadual, recursive = TRUE, showWarnings = FALSE)
dir.create(path_outputs_estadual, recursive = TRUE, showWarnings = FALSE)

# Configuração de paleta centralizada e sem problemas de encoding
paleta_cores <- c(
...
# 2. PROCESSAMENTO REFINADO ----------------------------------------------------
caminho_parquet <- file.path(path_raw_estadual, "msc_orcamentaria_estados_2019_2025_final.parquet")
if (!file.exists(caminho_parquet)) {
  stop("ERRO: O arquivo 'msc_orcamentaria_estados_2019_2025_final.parquet' não foi encontrado em data/raw/estadual/.")
}

df_msc <- read_parquet(caminho_parquet)
...
# Salva a base processada de estados
write_excel_csv2(df_estados_final, file.path(path_processed_estadual, "estadual_final.csv"))
...
g1 <- ggplot(df_resumo_nominal, aes(x = factor(exercicio), y = valor, fill = origem)) +
  ...
  labs(title = "Evolução do Investimento Cultural Estadual por Fonte (Valores Nominais)",
       subtitle = "R$ Bilhões", x = "Ano", y = "Valor Empenhado Nominal", fill = "Origem do Recurso") +
  theme_minimal() + theme(legend.position = "bottom", plot.title = element_text(face="bold", size = 14))
ggsave(file.path(path_outputs_estadual, "grafico_evolucao_nominal.png"), plot = g1, width = 10, height = 6)
...
g2 <- ggplot(df_resumo_real, aes(x = factor(exercicio), y = valor, fill = origem)) +
  ...
  labs(title = "Evolução do Investimento Cultural Estadual por Fonte (Valores Reais)",
       ...
       x = "Ano", y = "Valor Empenhado Real (R$ Bilhões)", fill = "Origem do Recurso") +
  theme_minimal() + theme(legend.position = "bottom", plot.title = element_text(face="bold", size = 14))
ggsave(file.path(path_outputs_estadual, "grafico_evolucao_real.png"), plot = g2, width = 10, height = 6)
...
g3 <- ggplot(df_resumo_percentual, aes(x = factor(exercicio), y = participacao, fill = origem)) +
  ...
  labs(title = "Composição Percentual do Investimento Cultural Estadual por Fonte",
       ...
       x = "Ano", y = "Percentual (%)", fill = "Origem do Recurso") +
  theme_minimal() + theme(legend.position = "bottom", plot.title = element_text(face="bold", size = 14))
ggsave(file.path(path_outputs_estadual, "grafico_composicao_percentual.png"), plot = g3, width = 10, height = 6)
...
# 5. RECEITA CORRENTE LÍQUIDA E PAINEL DE ESFORÇO FISCAL -----------------------
arquivos_rcl_est <- list.files(path_raw_estadual, pattern = "^RCL_.*_Estados\\.csv$", full.names = TRUE)
  ...
  map_dfr(function(arq) {
    ...
    if (ano_arq >= 2019 & ano_arq <= 2024) {
      ...
      read_delim(arq, delim = ";", skip = linha_header - 1, 
        ...
        clean_names() %>%
        filter(str_detect(tolower(conta), "receita corrente líquida \\(iii\\)"),
               str_detect(tolower(coluna), "total \\(últimos 12 meses\\)")) %>%
        mutate(exercicio = ano_arq,
               valor_rcl = as.numeric(valor),
               uf_chave = str_trim(toupper(uf))) %>% 
        select(exercicio, uf_chave, valor_rcl)
    }
  })

# Preparar dados de cultura garantindo padronização
df_cultura_proprio_est <- df_estados_final %>%
  filter(origem == "Recurso Próprio (Estadual)") %>%
  group_by(exercicio, uf_chave = toupper(uf)) %>%
  summarise(total_cultura_proprio = sum(valor_nominal_final, na.rm = TRUE), .groups = "drop")

# Join flexível e preenchimento de lacunas
df_meta_pnc_est <- df_cultura_proprio_est %>%
  full_join(df_rcl_estados, by = c("exercicio", "uf_chave")) %>%
  rename(uf = uf_chave) %>%
  complete(exercicio = 2019:2024, uf) %>%
  mutate(
    total_cultura_proprio = replace_na(total_cultura_proprio, 0),
    valor_rcl = replace_na(valor_rcl, 0),
    percentual_rcl = ifelse(valor_rcl > 0, (total_cultura_proprio / valor_rcl) * 100, 0),
...
g4 <- ggplot(df_painel_estados, aes(x = factor(exercicio), y = uf, fill = percentual_rcl)) +
  ...
  labs(title = "Painel de Esforço Fiscal da Cultura nas Esferas Estaduais (2019-2024)",
       ...
       caption = "Fonte: MSC/SICONFI.") +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 13, color = "#1b2631"),
        plot.subtitle = element_text(size = 12, color = "#566573"),
        axis.text.x = element_text(face = "bold", size = 11),
        axis.text.y = element_text(face = "bold", size = 11, color = "#1b2631"),
...
ggsave(file.path(path_outputs_estadual, "painel_esforco_fiscal.png"), plot = g4, width = 11, height = 7)
