...
# 
# INPUTS:
# - Bases processadas em memória dos scripts anteriores (Federal, Estadual, Municipal)
# 
# OUTPUTS:
...
# 1. CONSOLIDAÇÃO DOS DADOS NACIONAIS POR ESFERA --------------------------------

# 1.1 Esfera Municipal (Agregado Nacional por Ano e Fonte)
...
# 1.2 Esfera Estadual (Agregado Nacional por Ano e Fonte)
...
# 1.3 Esfera Federal (Agregado Nacional por Ano)
...
# 2. EMPILHAMENTO DA MATRIZ TRICOLOR (FEDERATIVA) ------------------------------
...
# 3. GRÁFICO 1: EVOLUÇÃO ABSOLUTA DO GASTO CULTURAL POR ESFERA DE GOVERNO -----
...
# 4. GRÁFICO 2: PERCENTUAL ANUAL (MUNICÍPIOS E ESTADOS USANDO APENAS RECURSO PRÓPRIO) ----
df_perc_municipal <- df_municipios_final %>%
  ...
  mutate(esfera = "Municipios (Próprio)")
...
df_tres_esferas_proprio <- bind_rows(df_perc_municipal, df_perc_estadual, df_perc_federal) %>%
  ...
  ungroup() %>%
  mutate(esfera = factor(esfera, levels = c("União (Pleno)", "Estados (Próprio)", "Municipios (Próprio)")))
