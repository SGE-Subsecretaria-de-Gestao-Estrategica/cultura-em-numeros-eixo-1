...
# 
# INPUTS:
# - data/processed/federal/federal_final.csv (ou df_anual_ajustado em memória)
# - data/processed/estadual/estadual_final.csv (ou df_estados_final em memória)
# - data/processed/municipal/municipal_final.csv (ou df_municipios_final em memória)
# 
# OUTPUTS:
...
# Garante a replicabilidade buscando as bases processadas em CSV ou
# executando os scripts das esferas fontes caso não estejam disponíveis.

# 1.1 Base Federal
if (!exists("df_anual_ajustado")) {
  file_fed <- here("data", "processed", "federal", "federal_final.csv")
  if (file.exists(file_fed)) {
    df_anual_ajustado <- read_csv2(file_fed, show_col_types = FALSE)
  } else {
    script_fed <- here("scripts", "federal", "gasto_federal.R")
    if (file.exists(script_fed)) {
      source(script_fed)
    } else {
      stop("ERRO: Base federal não encontrada em memória nem em data/processed/federal/federal_final.csv.")
    }
  }
}
...
if (!exists("df_estados_final")) {
  ...
  if (file.exists(file_est)) {
    ...
    if (file.exists(script_est)) {
      ...
      stop("ERRO: Base estadual não encontrada em memória nem em data/processed/estadual/estadual_final.csv.")
    }
  }
}
...
if (!exists("df_municipios_final")) {
  ...
  if (file.exists(file_mun)) {
    ...
    if (file.exists(script_mun)) {
      ...
      stop("ERRO: Base municipal não encontrada em memória nem em data/processed/municipal/municipal_final.csv.")
    }
  }
}
...
df_nacional_federal <- df_anual_detalhado %>%
...
df_perc_federal <- df_anual_detalhado %>%
  mutate(exercicio = as.numeric(ano)) %>%
  filter(exercicio >= 2019 & exercicio <= 2024) %>%
  group_by(exercicio) %>%
