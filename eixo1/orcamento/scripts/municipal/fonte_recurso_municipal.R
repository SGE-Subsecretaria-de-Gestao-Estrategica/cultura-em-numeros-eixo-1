#===============================================================================
# PROJETO CULTURA EM NÚMEROS - EIXO 1 (MUNICÍPIOS)
# SICONFI + DADOS BB ÁGIL (LAB 1) + RCL
#===============================================================================
...
path_base_siconfi <- "E:\\Pedro Buril\\Ministerio da Cultura\\Cultura em Numeros\\Eixo 1\\Orcamento\\SICONFI_MUNICIPAL\\municipios_parquet"
path_base_bb      <- "E:\\Pedro Buril\\Ministerio da Cultura\\Cultura em Numeros\\Eixo 1\\Orcamento\\LAB1 - MUNIC"
path_pai_rcl      <- "E:\\Pedro Buril\\Ministerio da Cultura\\Cultura em Numeros\\Eixo 1\\Orcamento\\SICONFI_MUNICIPAL"

# Paleta e Códigos Padrão
paleta_cores <- c("Recurso Próprio (Municipal)"      = "#1b2631", 
...
df_municipios_siconfi <- df_municipios_bruto %>%
  ...
  filter(uf_sigla != "DF") %>%
  mutate(origem = case_when(str_detect(fonte_string, "^1719|^2719|^1720|^2720|^719|^720") ~ "PNAB (Aldir Blanc 2)",
...
arquivo_lab <- file.path(path_base_bb, "cubo_execucao_lab1_municipios__2026-07-20_14-20.xlsx")
...
# 4. INTEGRAÇAO FINAL E DEFLACAO (SICONFI + BB ÁGIL) ---------------------------
...
tabela_conferencia_nt <- df_conferencia_totais %>%
  ...
  fmt_currency(columns = valor_total_exato,
    currency = "BRL",
    dec_mark = ",",
    sep_mark = ".",
    decimals = 2) %>%
  fmt_percent(columns = percentual,
    ...
    dec_mark = ",",
    sep_mark = ".") %>%
  grand_summary_rows(columns = valor_total_exato,
    fns = list("TOTAL GERAL" = ~sum(., na.rm = TRUE)),
    fmt = ~ fmt_currency(., currency = "BRL", dec_mark = ",", sep_mark = ".", decimals = 2)) %>%
  grand_summary_rows(columns = percentual,
    fns = list("TOTAL GERAL" = ~sum(., na.rm = TRUE)),
    fmt = ~ fmt_percent(., decimals = 2, dec_mark = ",", sep_mark = ".")) %>%
  tab_source_note(source_note = "Fonte: Ministério da Cultura / SICONFI e BBÁgil. Elaboração: Cultura em Números (2026).") %>%
  tab_options(heading.title.font.size = px(15),
    heading.subtitle.font.size = px(12),
    column_labels.font.weight = "bold",
    column_labels.background.color = "#f2f4f4",
    grand_summary_row.background.color = "#eaeded",
...
# Gráfico 2: Evolução Real (Base 2024)
...
# GrÃ¡fico 3: ComposiÃ§Ã£o Percentual
...
ggplot(df_resumo_percentual, aes(x = factor(exercicio), y = participacao, fill = origem)) +
  ...
  labs(title = "Composição Percentual do Investimento Cultural Municipal por Fonte",
       ...
       caption = "Fonte: MSC/SICONFI e BBÁgil. Deflação: IPCA/SGS-BCB (2024).") +
  theme_minimal() + 
...
#===============================================================================
# 6. ANÁLISE: RECEITA CORRENTE LÍQUIDA (META PNC)
#===============================================================================
