<script module lang="ts">
  ...
  import EquipamentosCulturaisChart from './EquipamentosCulturaisChart.svelte';

  const { Story } = defineMeta({
    title: 'Charts/Equipamentos Culturais',
    component: EquipamentosCulturaisChart,
    tags: ['autodocs'],
    parameters: {
      layout: 'padded',
    },
  ...
  type StoryArgs = ComponentProps<typeof EquipamentosCulturaisChart>;
</script>
...
{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <EquipamentosCulturaisChart {...args} />
  </StoryFrame>
{/snippet}

<Story name="Default" template={template as never} />
