<script module lang="ts">
  ...
  import TresEsferasChart from './TresEsferasChart.svelte';
  ...
  type StoryArgs = ComponentProps<typeof TresEsferasChart>;
</script>

{#snippet template(args: StoryArgs, ctx: StoryContext<StoryArgs>)}
  <StoryFrame name={ctx.id}>
    <TresEsferasChart {...args} />
  </StoryFrame>
{/snippet}

<!--
  As três esferas como faixas grossas no estilo do print de referência: sem
  eixo Y, valores escritos sobre os pontos, e o último valor de cada faixa
  como número grande no bloco de ponta.
