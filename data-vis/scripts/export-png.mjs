...
/**
 * The A4 proof page — one chart per sheet, at their authored sizes. `bg=0`
 ...
 */
const PAGE = '/a4.html?bg=0';
const PORT = 5233;

/** Output name per figure, in the order they appear on the page. */
const NAMES = [
...
try {
  ...
  if (charts.length !== NAMES.length) {
    throw new Error(`esperava ${NAMES.length} gráficos em ${PAGE}, encontrei ${charts.length}`);
  }
  ...
  if (failures.length) throw new Error(`erros na página: ${failures.join(' · ')}`);
} finally {
  await browser.close();
  await server.close();
}
